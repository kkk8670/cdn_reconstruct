...
<script>
import CreateUpdate from '@/v2/cdn_common/views/site-admin/advanced/components/RequestHeaders/createUpdate'
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.status && !tableData.length) {
        this.$msg.error('请添加自定义HTTP回源请求头')
        return false
      }

      return {
        proxy_set_header: JSON.stringify(tableData)
      }
    }
  }
}
