  ...
  <ul class="space-y-8">
    ...
    <li>
      <el-table :data="tableData" border size="small" max-height="400px">
        <ants-table-column label="黑名单IP" prop="ip" min-width="150">
        </ants-table-column>
...
<script>
import CreateUpdate from '@/v2/cdn_common/views/site-admin/application-secure/components/IpBlack/createUpdate'
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.status && !tableData.length) {
        this.$msg.error('请添加IP黑名单')
        return false
      }

      return {
        server_waf_black_ip: JSON.stringify(tableData)
      }
    }
  }
}
