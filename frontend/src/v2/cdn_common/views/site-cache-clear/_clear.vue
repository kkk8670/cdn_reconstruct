    ...
    <div class="my-lg">
      ...
      >
        <ants-button
          ...
          icon="el-icon-plus"
          :text="buttonText"
          class="mb-lg max-w-full suffix-button"
        />
      </FileExtension>
      ...
      <ants-input
        ...
        :placeholder="
          `${antsT('请输入')} ${antsT(
            typeObj[form.activeType].placeholder
...
export default {
  ...
  computed: {
    buttonText() {
      return this.form.suffix.join('|') || this.antsT('请选择文件类型')
    }
  },
  created() {},
  methods: {
    ...
    async submitFrom() {
      if (this.form.content == '') {
        return this.$msg.error(
          `${this.antsT('请输入')} ${this.antsT(
            this.typeObj[this.form.activeType].placeholder
          )} ${this.antsT('内容')}`
        )
      }
      ...
      if (this.form.activeType === 'suffix') {
        if (!this.form.suffix.length) return this.$msg.error(this.antsT('请选择文件类型'))
      }
