...
const { doReauthCertify } = require(`@/v2/${PROCESS_NAME}/api/cdn/site`)
const defaultForm = {
  commonName: '',
  id: null,
  // 0 = 默认证书 | 1 = ZeroSSl
...
export default {
  ...
  methods: {
    show({ id, ids, srcType = certificateType[0].value, commonName }) {
      console.log('srcType', srcType)
      this.visible = true
      this.form.commonName = commonName
      this.form.id = id
      this.form.ids = ids
      this.form.type = srcType || certificateType[0].value
    },
    ...
    async submit() {
      ...
      try {
        const { data: res } = await doReauthCertify(this.form)
