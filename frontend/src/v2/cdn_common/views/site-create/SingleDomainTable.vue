...
export default {
  ...
  computed: {
    // 判断源站域名跟加速域名是否重复
    isRepeatDomains() {
      return this.tableForm.some((item) => {
        if (item.domain && this.domains.includes(item.domain.trim())) {
          return true
        }
      })
    },
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          this.$refs.tableFormRef.validate((valid) => {
            ...
            const output = []
            this.tableForm.forEach((item) => {
              const { domain, port, portHttps, weight } = item

              let portStr = port
              let isValid = domain && port && weight
              if (this.sProtocol === '$scheme') {
                portStr = `${port},${portHttps}`
                isValid = domain && port && portHttps && weight
              }

              if (isValid) {
                output.push(`${domain}|${portStr}|${weight}`)
              }
            })
