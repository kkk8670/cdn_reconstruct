...
export const flow = {
    ...
    unit: 'KB',
    queryObj: (form = {}) => {
        const QUERY = encodeURIComponent(`sum(increase(nginx_vts_server_bytes_total{host=~"${getHost(form.host)}"}[${form.step}s])) by (direction)`)
...
export const bandwidth = {
    ...
    unit: 'Kbps',
    queryObj: (form = {}) => {
        const QUERY = encodeURIComponent(`sum(quantile_over_time(0.95,nginx_vts_server_bytes_total{host=~"${getHost(form.host)}"}[${form.step}s])) by (direction)`)
        return buildQueryString(form, QUERY)
    },
    // 计算数据
    computedValue: (value) => +value,
...
export const request = {
    ...
    unit: '次',
    queryObj: (form = {}) => {
        const QUERY = encodeURIComponent(`ceil(sum(increase(nginx_vts_server_requests_total{host=~"${getHost(form.host)}"}[${form.step}s])))`)
        return buildQueryString(form, QUERY)
    },
