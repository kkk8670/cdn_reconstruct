...
export default {
  ...
  methods: {
    ...
    async changeStatus(row, switchRef) {
      ...
      const { data: res } = await addOutRdata(row)
    ...
    async getTableData() {
      ...
      try {
        const { data: res } = await getOutList(this.queryInfo)
    ...
    async delRdata(id) {
      ...
      if (!result) return
      const { data: res } = await deleteOutRdata({ ids: id })
      if (res.code !== 1) return
    ...
    async deleteRdata() {
      ...
      const { data: res } = await deleteOutRdata({ ids })
      if (res.code !== 1) return
      this.$msg.success('删除成功')
      this.getTableData()
    }
