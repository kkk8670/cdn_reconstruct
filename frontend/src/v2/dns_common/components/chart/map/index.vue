...
// const { getQpsChart } = require(`@/v2/${PROCESS_NAME}/api/dns/monitor`)
  ...
  methods: {
    ...
    getChart(query = {}) {
      this.$nextTick(async () => {
        ...
        // this.loading = true
        // const { data: res } = await getQpsChart(this.queryData.queryObj(QUERY))
        // this.loading = false
        ...
        if (
          ...
          chartData.data.result.forEach((item, idx) => {
            const nowValue = Math.ceil(item.value)
            ...
            if (nowValue != 0) {
              ...
              //     ''
              //   )
              const nowName = item.name.replace(
                /市|省|壮族自治区|回族自治区|维吾尔族自治区|自治区/g,
                ''
              )
              // 过滤掉其他国家的省区
              if (QUERY.area === 'china') {
                if (provinceArr.indexOf(nowName) != -1) {
                  topTableData.push({
                    name: nowName,
                    value: nowValue
                  })
                }
                // } else if (QUERY.area === 'city') {
