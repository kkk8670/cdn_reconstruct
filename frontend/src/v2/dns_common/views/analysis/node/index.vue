...
export default {
  ...
  methods: {
    ...
    async getNodes() {
      this.loading = true
      try {
        const { data: res } = await getServerList({
          key: this.query.name.trim(), // 搜索框内容
          groupId: '', // 分组ID
          sidx: 'id', // 排序字段
        ...
        this.query.nodes = nodes.map(item => item.ip)
