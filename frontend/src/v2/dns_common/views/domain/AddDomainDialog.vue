<template>
  ...
  >
    ...
    >
      <ants-form-item prop="user" class="mb-lg" v-if="isAdmin">
        <el-select
          v-model="form.user"
          filterable
          remote
          ...
          style="width: 100%;"
          value-key="userId"
          class="query-user-select el-icon-user-solid ants-select-bgc"
        >
          <el-option
...
export default {
  ...
  data() {
    ...
    const checkDomain = (rule, value, callback) => {
       ...
       */
      const regDomain = /^(?=^.{3,255}$)[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(?<!www)(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+$/
