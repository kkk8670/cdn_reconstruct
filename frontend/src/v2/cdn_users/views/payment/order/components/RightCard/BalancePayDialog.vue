<template>
  ...
  >
    <!-- <div class="font-semibold text-center mb-xl">
      {{ antsT('请输入支付密码进行支付，') }}<router-link to="/cdn/account/info"
        >{{ antsT('还未设置支付密码？') }}</router-link
      >
    </div>
    <ants-form
      :model="form"
...
<script>
// API
import { balancePay } from '@/v2/cdn_users/api/payment'
...
export default {
  ...
  methods: {
    ...
    showDialog(orderId) {
      this.form.serialNumber = orderId
    ...
    async submitPayByBalance() {
      ...
      this.$router.replace('/cdn/finance/order/14')
      // })
    },
