    ...
    <el-skeleton :rows="8" :loading="$root.loading" animated>
      <template>
          ...
          <li v-for="(item, key) in priceUsbleObj" :key="key">
            <div class="pr-lg" v-if="priceName[key]">
              ...
              <div class="text-ants-text-4 line-through">
                {{ (item.value * 2) | priceFormat }}/{{ antsT(priceName[key]) }}
              </div>
            </div>
          </li>
        </ul>
        ...
        <div class="p-lg mt-lg bg-ants-bg-2 rounded-2xl text-sm">
          <div class="mb-lg">
            <strong class="mr-lg">{{ antsT('计费方式') }}</strong>
            <el-tag size="small" type="primary">{{ antsT('流量月结') }}</el-tag>
          </div>
          <div class="mb-lg">
            <span class="inline-block mr-sm" style="width: 100px">{{
              antsT('流量规格')
            }}</span>
            <strong v-if="activePlanItem && activePlanItem.attr">
              {{ flowUnit(activePlanItem.attr.flow) }}/{{ antsT('月') }}</strong
            >
          </div>
          <PlanAttr
            :attr-obj="activePlanItem && activePlanItem.attr"
            class="space-y-8"
          />
        </div>
...
export default {
  ...
  computed: {
    ...
    activePlanItem() {
      const obj = {
        attr: {},
        priceObj: {},
        ...((this.form.plan && this.planObj[this.form.plan]) || {})
      }
  ...
  methods: {
    ...
    async getPlanListData() {
      ...
      if (!res.data.length) {
        return this.$msg.error(this.antsT('暂无可购买套餐，请联系客服！'))
      }

      const obj = {}
      const firstPlanIds = []
      console.log(firstPlanIds[0])
      res.data.forEach((item) => {
        // 过滤掉带宽计费的套餐，带宽计费的为后付费
        if (item.attr && item.attr.charging_mode == 1) {
          firstPlanIds.push(item.id)
          obj[item.id] = {
            ...item,
            // 套餐价格
      ...
      this.planObj = obj
      // 初始值
      this.form.plan = this.$route.query.id || firstPlanIds[0]
      // 更新父组件的数据
