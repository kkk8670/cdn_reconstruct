  ...
  >
    ...
    >
      ...
      <ants-form-item label="选择套餐" prop="serialNumber">
        <QueryPlanSelect v-model="form.serialNumber" ref="QueryPlanSelectRef"
        :disabled="!!form.id" />
      </ants-form-item>

      <!-- 修改 -->
      <!-- <ants-form-item v-else label="绑定套餐" required>
        <ants-input v-model="form.plan" disabled></ants-input>
      </ants-form-item> -->
      ...
      <ants-form-item label="PP协议" required prop="proxy_protocol">
        ...
        <el-tooltip
          ...
          placement="top-start"
        >
          <span class="el-icon-info pl-df cursor-color"></span>
        </el-tooltip>
      </ants-form-item>
    ...
    showDialog(row = {}) {
      ...
      if (row.id) {
        ...
        this.form = {
          ...
          user: row.user,
          // 套餐名称
          plan: row.suit && row.suit.product && row.suit.product.name,
