<template>
  <el-card class="mb-lg content-box">
    ...
    <el-skeleton :rows="8" :loading="!$root.userinfo.userId" animated>
      ...
      >
        <ants-form-item prop="account" label="DNS登录账号">
          <ants-input
            v-model="form.account"
            placeholder="请输入DNS登录账号"
            prefix-icon="el-icon-user"
          />
        </ants-form-item>

        <ants-form-item prop="password" label="DNS登录密码">
          <ants-input
            v-model="form.password"
            type="password"
            placeholder="请输入DNS登录密码"
...
// API
import { cdnAccessToken, dnsAccessToken } from '@/v2/cdn_users/api/cdn/account'
...
export default {
  ...
  computed: {
    ...
    accessInfo({ $root }) {
      ...
      return { my: ucdnAccessToken, other: udnsAccessToken }
    }
  },
  watch: {
    accessInfo: {
    ...
    async login() {
      this.$refs.formRef.validate(async valid => {
        ...
        try {
          ...
          const { data: res } = await dnsAccessToken({
            account,
            password
          })
