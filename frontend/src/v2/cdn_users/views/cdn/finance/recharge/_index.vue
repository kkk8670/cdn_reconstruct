<template>
  <div class="recahrge-box animated fadeIn">
    ...
    <WechatPayDialog
      ...
      back-url="/cdn/finance/recharge"
      @refresh="resetData"
    />
    <IframeDailog ref="IframeDailogRef" />
  </div>
</template>
...
import WechatPayDialog from '@/v2/cdn_users/views/payment/wechat'
import IframeDailog from './IframeDailog'
...
export default {
  ...
  methods: {
    ...
    async toRecharge() {
      ...
      // 如果没有选择金额
      if (this.payPrice <= 0) {
        return this.$msg.error('请选择或输入您要充值的金额！')
      }
      ...
      // 已开启的支付方式
      const { alipay, wechat, stripe, tokenpay, cccyun, allinpay } =
        this.powerObj
    ...
    async createOrder() {
      ...
      try {
        const { data: res } = await doCreateOrder({
          ...
          // 目标ID，这里置为 1
          targetId: 1,
          initJson: JSON.stringify({ sum: this.payPrice })
        })
        ...
        return {
          ...
          body: '余额充值',
          // 订单号
          outTradeNo: obj.serialNumber,
          // 支付金额，以分为单位
          totalAmount: obj.payable
        }
      } catch (err) {
        console.log(err)
        return false
    ...
    async _payByAlipay(orderInfo = {}) {
      // this.$refs.IframeDailogRef.show(orderInfo)
    },
