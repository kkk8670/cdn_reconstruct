    ...
    <ul>
      <li class="border-b border-ants-border-4 pb-lg">
        ...
        <div class="text-sm text-ants-text-4 mt-sm">
          {{antsT('提交时间')}}：{{ orderData.createdate }}
        </div>
      </li>
      <li
        class="flex justify-between items-center py-lg border-b border-ants-border-4"
      >
        <span class="mr-lg">{{antsT('工单编号')}}</span
        ><span class="text-sm">{{ orderData.id }}</span>
      </li>
      <li
        class="flex justify-between items-center py-lg border-b border-ants-border-4"
      >
        <span class="mr-lg">{{antsT('相关产品')}}</span
        ><span class="one_show-h text-sm">{{ orderData.categoryObj && orderData.categoryObj.title }}</span>
      </li>
      <li
        class="flex justify-between items-center py-lg border-b border-ants-border-4"
      >
        <span class="mr-lg">{{antsT('紧急程度')}}</span>
        <el-tag
          size="mini"
          :type="orderData.urgentLevel > 2 ? 'danger' : 'primary'"
        >
          {{ antsT(urgencyObj[+orderData.urgentLevel]) }}
        </el-tag>
      </li>
      <li
        class="flex justify-between items-center py-lg border-b border-ants-border-4"
      >
        <span class="mr-lg">{{antsT('当前状态')}}</span>
        <span
          class="text-sm"
          :class="[orderStatusObj[orderData.status].icon]"
          :style="{ color: orderStatusObj[orderData.status].color }"
        >
          {{ antsT(orderStatusObj[orderData.status].txt) }}</span
        >
      </li>
...
export default {
  ...
  methods: {
    showCloseOrderDialog() {
      this.$emit('showCloseOrderDialog')
    },
    showComplainDialog() {
