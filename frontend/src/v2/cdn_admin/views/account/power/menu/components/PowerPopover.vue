<template>
  ...
  >
    <div class="flex items-center justify-between">
      <strong>{{ antsT(menuRowData.name) }}</strong>
      <ants-button
        size="mini"
        icon="el-icon-plus"
        type="primary"
    ...
    >
      ...
      >
        <ants-table-column min-width="100" prop="name" label="权限名称">
        ...
        <ants-table-column min-width="120" prop="perms" label="权限标识">
          <template #default="{ row }">
        ...
        <ants-table-column width="90" label="操作">
          <template #default="{ row, $index }">
            ...
            <div v-else>
              ...
              <el-divider direction="vertical"></el-divider>
              <span
                class="text-ants-text-4 cursor-pointer"
...
export default {
  ...
  methods: {
    ...
    confirmChangeTb(row) {
      if (!this.form.name) return this.$msg.error(this.antsT('请输入权限名称'))
      if (!this.form.perms) return this.$msg.error(this.antsT('请选择权限'))
      this.$refs.formRef.validate(async valid => {
        ...
        this.$msg.success(this.antsT(`${!this.form.menuId ? '添加' : '修改'}权限成功`))
      })
    },
    ...
    async deletePermissions(permsArr = [], row, idx) {
      ...
      this.$msg.success(this.antsT('删除权限成功'))
    },
