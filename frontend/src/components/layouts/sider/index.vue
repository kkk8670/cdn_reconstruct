<template>
  ...
  >
    ...
    <AntsScrollerMenu :iscollapse="iscollapse">
      <template v-for="item of routes">
        <component
          :is="item.meta.isSub ? 'AntsSubMenu' : 'AntsMenuItem'"
          :key="item.path"
          :item="item.meta.isSub ? item : [item]"
...
export default {
  ...
  computed: {
    ...
    routes({ $route }) {
      ...
      return this.routesObj[ROUTER_NAME].filter(item => !item.meta.hidden)
    }
  },
  created() {},
  methods: {
